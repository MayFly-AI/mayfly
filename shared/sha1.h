...
class ISha1 {
	public:
		...
		virtual void Update(const uint8_t* pbData,uint32_t uLen)=0;
		virtual bool GetHash(uint8_t* pbDest20) const=0;
		virtual void Final()=0;
		static ISha1* Create();
		static void Destroy(ISha1* pSha1);
};
...
class CHMAC_SHA1
{
	...
	public:
        ...
        void HMAC_SHA1(const uint8_t *text,int text_len,const uint8_t *key,int key_len,uint8_t *digest);
};
...
public:
	...
	virtual void Update(const UINT_8* pbData, UINT_32 uLen);
	...
	virtual bool GetHash(UINT_8* pbDest20) const;
