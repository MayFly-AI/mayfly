...
void ISha1::Destroy(ISha1*pSha1) {
	delete (CSHA1*)pSha1;
}
ISha1::~ISha1() {}


CHMAC_SHA1::CHMAC_SHA1() :szReport(new char[HMAC_BUF_LEN]),
...
void CHMAC_SHA1::HMAC_SHA1(const uint8_t *text,int text_len,const uint8_t *key,int key_len,uint8_t *digest) {
	...
	if(key_len>SHA1_BLOCK_SIZE) {
		pSha1->Reset();
		pSha1->Update((uint8_t*)key,key_len);
		pSha1->Final();

		pSha1->GetHash((uint8_t*)SHA1_Key);
	} else
	...
	/* STEP 4 */
	pSha1->Reset();
	pSha1->Update((uint8_t*)AppendBuf1,sizeof(m_ipad)+text_len);
	pSha1->Final();

	pSha1->GetHash((uint8_t*)szReport);
	...
	/*STEP 7 */
	pSha1->Reset();
	pSha1->Update((uint8_t*)AppendBuf2,sizeof(m_opad)+SHA1_DIGEST_LENGTH);
	pSha1->Final();

	pSha1->GetHash((uint8_t*)digest);
