...
__global__ static void YuvToRgbKernelAlpha(uint8_t *pYuv, int nYuvPitch, uint8_t *pRgb, int nRgbPitch, int nWidth, int nHeight,int alpha32) {
    ...
    *(RgbIntx2 *)pDst = RgbIntx2 {
        YuvToRgbForPixel<Rgb>(l0.x, ch.x, ch.y).d|alpha32,
        YuvToRgbForPixel<Rgb>(l0.y, ch.x, ch.y).d|alpha32,
    };
    *(RgbIntx2 *)(pDst + nRgbPitch) = RgbIntx2 {
        YuvToRgbForPixel<Rgb>(l1.x, ch.x, ch.y).d|alpha32, 
        YuvToRgbForPixel<Rgb>(l1.y, ch.x, ch.y).d|alpha32,
    };
}
...
template <class COLOR32> void Nv12ToColor32Alpha(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, int iMatrix,uint8_t alpha) {
    SetMatYuv2Rgb(iMatrix);
	int alpha32=(int)alpha<<24;
    YuvToRgbKernelAlpha<uchar2, COLOR32, uint2>
        <<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2 / 2), dim3(32, 2)>>>
        (dpNv12, nNv12Pitch, dpBgra, nBgraPitch, nWidth, nHeight,alpha32);
}
...
template void Nv12ToColor32Alpha<RGBA32>(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, int iMatrix,uint8_t alpha);
